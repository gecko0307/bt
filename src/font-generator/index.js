...
async function generateFonts(req) {
    ...
    try {
        ...
        for (const fontFile of Object.keys(config)) {
            ...
            const font = await fonteditorcore.generate(fontsPath, fontFile, fontOptions);
            
            // TODO: validate fontOptions.fontname length
            
            if (fontOptions["toFile"] === true) {
                const fontFilename = `${fontOptions.fontname}.woff`;
                const fontPath = path.resolve(`./HTML/assets/${fontFilename}`);
                const fontBuffer = font.write({
                    type: "woff"
                });
...
`@font-face {
    ...
    font-weight: normal;
    font-style: normal;
    src: url("assets/${fontFilename}") format("woff");
}
`;
                css += fontFace;
            }
            else {
                const fontDataUrl = font.toBase64({
                    type: "woff"
                });
                const fontFace = 
`@font-face {
